...
结合 hotpotqa_v3.py 的数据处理逻辑和 GAM 框架，测试在多跳推理数据上的效果。
"""
...
# ========== Prompt 设计：完全借鉴自 hotpotqa_v3.py ==========
...
def answer_with_summary(summary: str, question: str, generator) -> str:
    """基于研究摘要回答问题"""
    prompt = make_summary_prompt(summary, question)
    raw = generator.generate_single(prompt=prompt)
    return raw.get("text", "").strip()

def answer_with_memory(final_memory: Dict[str, Any], question: str, generator) -> str:
    """基于记忆状态回答问题"""
...
def process_sample(sample: Dict[str, Any], sample_index: int, outdir: str):
    ...
    3. 基于研究结果进行问答
    """
    sample_id = sample.get("_id", f"sample_{sample_index}")
    ...
    try:
        ...
        print(f"段落数: {len(session_chunks)}")
        if session_chunks:
        ...
        # 4. 使用 MemoryAgent 构建记忆（将每个段落作为一条消息）
        ...
        for i, session_chunk in enumerate(session_chunks, 1):
            print(f"  处理段落 {i}/{len(session_chunks)}...")
        ...
        for i, qi in enumerate(qas, 1):
            ...
            q_type = qi.get("type")
            level = qi.get("level")
            supporting_facts = qi.get("supporting_facts", [])
            ...
            try:
                ...
                # 基于研究结果生成答案
                print("生成答案...")
                summary_answer = answer_with_summary(research_summary, q, generator)
                memory_answer = answer_with_memory(final_memory_str, q, generator)
...
def main():
    ...
    parser.add_argument("--data", type=str, default="/share/project/bingyu/datasets/hotpotqa/hotpot_dev_distractor_v1.json", 
                        help="HotpotQA 数据集路径")
    parser.add_argument("--outdir", type=str, default="/share/project/bingyu/code/general-agentic-memory/results/hotpotqa_output",
                        help="输出目录")
    parser.add_argument("--start-idx", type=int, default=200, help="开始样本索引")
    parser.add_argument("--end-idx", type=int, default=2000, help="结束样本索引（不包含），None表示处理所有样本")
